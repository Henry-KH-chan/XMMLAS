...
def getLaueSpots(wavelmin, wavelmax, crystalsParams, kf_direction=DEFAULT_TOP_GEOMETRY,
    ...
    if verbose:
        #print("energy range: %.2f - %.2f keV" % (CST_ENERGYKEV / wlM, CST_ENERGYKEV / wlm))    khchan edited
    ...
    for i in list(range(nb_of_grains)):
        ...
        if verbose:
            #print("# grain:  ", i, " made of ", key_material)    khchan edited
    ...
    for i in list(range(nb_of_grains)):
        ...
        if fastcompute == 0:
            ...
            if verbose:
                pass
                #print("Orientation matrix", Orientmatrix)    khchan edited
                #print("# grain: ", i)
...
def filterLaueSpots(vec_and_indices, HarmonicsRemoval=1,
    ...
    if fastcompute == 0:
        ...
        HKLslist = emptylists(nbofgrains)
    ...
    for grainindex in list(range(nbofgrains)):
        ...
        h = np.compress(onCam_cond, HKLs_list[0][:,0])
        k = np.compress(onCam_cond, HKLs_list[0][:,1])
        l = np.compress(onCam_cond, HKLs_list[0][:,2])
        hkl = np.concatenate((h[:,None],k[:,None],l[:,None]), axis = 1)
        ...
        #print('oncam_chi',oncam_chi/DEG)    khchan edited
    ...
    if fastcompute == 0:
        ...
        return (np.concatenate(Oncam2theta) / DEG, np.concatenate(Oncamchi) / DEG, hkl)
